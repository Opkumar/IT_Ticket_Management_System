...
module.exports.updateUserProfile = async ({
  ...
  if (todayAcceptedTickets !== undefined)
    updateFields.todayAcceptedTickets = todayAcceptedTickets;
  if (todayCompletedTickets !== undefined)
    updateFields.todayCompletedTickets = todayCompletedTickets;
  if (totalAcceptedTickets !== undefined)
    updateFields.totalAcceptedTickets = totalAcceptedTickets;
  if (totalCompletedTickets !== undefined)
    updateFields.totalCompletedTickets = totalCompletedTickets;
  ...
  // Handle sentiments as array of numbers
  if (sentiments !== undefined) {
    if (!Array.isArray(sentiments)) {
      throw new Error("Sentiments must be an array of numbers");
    }

    if (sentiments.every((s) => typeof s === "number")) {
      // Push multiple sentiments
      updateFields.$push = { sentiments: { $each: sentiments } };
    } else {
      throw new Error("All sentiment values must be numbers");
    }
  }
