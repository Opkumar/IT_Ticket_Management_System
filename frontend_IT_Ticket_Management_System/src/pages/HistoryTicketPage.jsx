...
function HistoryTicketPage() {
  ...
  return (
    <div className="min-h-[calc(100vh-76px)]">
      ...
      {view ? (
        <div className="">
          {ticketTracking.filter((ticket)=> ticket.completedIssue).
          length > 0 ? (
            tractView ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 m-5">
                {ticketTracking.filter((ticket)=> ticket.completedIssue).map((ticket, index) => (
                  <div key={index}>
                    <div
                      key={index}
        ...
        <RequirementComponent data={true} />
      )}
    </div>
  );
}
