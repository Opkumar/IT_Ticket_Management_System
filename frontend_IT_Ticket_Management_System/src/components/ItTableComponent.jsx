...
const ItTable = ({ itrole, itAssignedView,ticketDataOfRow ,allUsers,allTickets}) => {
  ...
  const assignTicket = async (userId) => {
    try {
      ...
      const todayAcceptedTickets = allTickets.filter(
        (ticket) => ticket.acceptedTicketByUserId === userId && ticket.assigned && isSameDay(new Date(ticket.assignedAt), today)
      );
  
      const todayCompletedTickets = allTickets.filter(
        (ticket) => ticket.acceptedTicketByUserId === userId && ticket.completedIssue && isSameDay(new Date(ticket.completedTime), today)
      );
